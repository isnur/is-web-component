...
const patchEsm = () => {
    ...
    if (BUILD.cssVarShim && !(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return __sc_import_is_web_component('./css-shim-206ea950-3169f23e.js').then(() => {
...
const patchBrowser = async () => {
    ...
    if (importMeta !== '') {
        ...
        if (!window.customElements) {
            // @ts-ignore
            await __sc_import_is_web_component('./dom-96781eef-a2fb04dd.js');
        }
...
const createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
    ...
    if (isDef(newVNode.$text$)) {
        ...
        newVNode.$elm$ = (BUILD.isDebug || BUILD.hydrateServerSide) ? doc.createComment(`slot-reference:${hostTagName.toLowerCase()}`) : doc.createTextNode('');
    }
    else {
        // create element
